...
A simple example in pure NumPy is shown in \coderef{code:integer-encoding}.
...
\end{lstlisting}
The code in \coderef{code:pca} shows how to use the PCA implementation of scikit-learn in Python.
...
In most cases of high-dimensional data it is advisable to apply PCA to reduce the dimensionality of the data set.
...
The code for this is straight forward and shown in \coderef{code:lda-inference}.
...
\end{lstlisting}
The code in \coderef{code:tsne} shows how to use the t-SNE implementation of scikit-learn in Python.
...
print(bow)
\end{lstlisting}
The code in \coderef{code:bow} shows how to use the BoW-Feature implementation of scikit-learn in Python.
...
Because the \lstinlinde{text} variable is used at the end to generate the BoW-Feature vector, the resulting bow vector does not contain any zeros.
...
\begin{lstlisting}[language=Python, caption={word-frequency histogram for Moby Dick}, label={code:moby-raw}]
from sklearn.feature_extraction.text import CountVectorizer
...
\begin{lstlisting}[language=Python, caption={Reduced word-frequency histogram for Moby Dick excluding stop words.}, label={code:moby-stopwords}]
...
A possible implementation of this method is shown in \coderef{code:rolling-statistics}.
...
\end{lstlisting}
The code in \coderef{code:rolling-statistics} will load the Google stock price data set (\lstinline{2023/01/01 - 2024/01/01}) and then compute the rolling mean of the last 10 data points.
...
The Fourier Transform is a way to transform a function from the time domain to the frequency domain.
...
We can see an example usage of the Fourier Transform in \coderef{code:fourier-transform} and a visualization of the Fourier Transform in Figure \ref{fig:fourier-transform}.
